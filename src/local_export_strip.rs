                ...
                ModuleItem::ModuleDecl(mut module_decl) => {
                    ...
                    match module_decl {
                        ...
                        ModuleDecl::ExportNamed(export) => {
                            if let Some(ExportSpecifier::Named(_)) = export.specifiers.first() {
                                list.push(re_export(export, self.unresolved_mark).into())
                            } else {
                                // TODO: verify we don't need to worry about `configurable` for
                                // `ExportSpecifier::Namespace` (`export * as foo from './src`)
                                continue;
                            }
                        }
                    ...
                    ExportSpecifier::Named(ExportNamedSpecifier { orig, exported, .. }) => {
                        ...
                        if let Some(exported) = exported {
                            ...
                            if re_exported {
                              ...
                              (exported, Ident { span: DUMMY_SP, sym, optional: false })
                            } else {
                              (exported, orig)
                            }
                        } else {
